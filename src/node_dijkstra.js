  ...
  function path(startNode, endNode) {
    ...
    while (ALGORITHM) {
      ...
      for (var i = 0; i < currentNode.edges.length; i++) {
        ...
        if (
          currentNode.edges[i].c1 != currentNode &&
          !currentNode.edges[i].c1.visited
        ) {
          if (
            currentNode.distance + length <
            currentNode.edges[i].c1.distance
          ) {
            currentNode.edges[i].c1.distance = currentNode.distance + length;
          }
          connectedNodes.push(currentNode.edges[i].c1);
        } else if (
          currentNode.edges[i].c2 != currentNode &&
          !currentNode.edges[i].c2.visited
        ) {
          if (
            currentNode.distance + length <
            currentNode.edges[i].c2.distance
          ) {
      ...
      var bestConnected = connectedNodes.sort(function (a, b) {
